...
def _build_insights_prompt(user_prompt: str, schema_ctx: str, agg_summary: str) -> str:
    return (
        ...
        "Si un agrégat contient une série temporelle (plusieurs périodes triées — mois, "
        "trimestre, année), privilégie des insights de type 'trend' CHIFFRÉS plutôt que des "
        "commentaires généraux :\n"
        "  - Variation en % entre la première et la dernière période de la série\n"
        "  - Accélération/ralentissement récent : variation de la dernière période vs "
        "l'avant-dernière\n"
        "  - Si un agrégat catégoriel (région, catégorie, segment) est aussi disponible, "
        "le contributeur principal en % du total\n"
        "  - N'avance une explication causale (ex: un événement, un segment, un lancement) que "
        "si elle ressort directement d'une CORRÉLATION VISIBLE entre deux agrégats fournis — "
        "sinon ne pas en inventer, se limiter au constat chiffré\n\n"
        "Style attendu pour 'description' d'un insight de type trend (à adapter aux chiffres "
        "réels, ne jamais copier ces valeurs) :\n"
        '  "Le chiffre d\'affaires a progressé de +43 % entre la première et la dernière '
        "période, porté par une accélération sur la période la plus récente (+25 % vs période "
        'précédente). L\'Île-de-France reste le premier contributeur avec 38 % du total."\n\n'
        "Règles strictes :\n"
        "  - Ne jamais inventer de chiffres — chaque pourcentage doit être calculable depuis "
        "les données agrégées fournies ci-dessus\n"
        "  - Si une donnée manque pour un calcul (ex: pas de série temporelle disponible), "
        "l'indiquer dans 'supporting_data' plutôt que d'improviser\n"
